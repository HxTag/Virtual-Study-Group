...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['group_picture']) && $_FILES['group_picture']['error'] === 0) {
        try {
            error_log("Attempting to upload group picture. File details: " . print_r($_FILES['group_picture'], true));
            $new_group_picture = uploadGroupPicture($_FILES['group_picture'], $group_id);
            error_log("Group picture uploaded successfully. New URL: " . $new_group_picture);
        } catch (Exception $e) {
            error_log("Error uploading group picture: " . $e->getMessage());
            echo json_encode(["status" => "error", "message" => $e->getMessage()]);
            exit();
        }
    } else {
        error_log("No new group picture uploaded or upload error. Error code: " . ($_FILES['group_picture']['error'] ?? 'no file'));
