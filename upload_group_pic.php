...
function uploadGroupPicture($file, $group_id)
{
    ...
    $ch = curl_init();
    $uploadUrl = rtrim($minioHost, '/') . '/' . $minioBucketName . '/' . $uniqueFileName;
    error_log("Attempting to upload to URL: " . $uploadUrl);
    ...
    error_log("MinIO Upload Debug Info: " . $verboseLog);
    error_log("MinIO Upload Response Code: " . $httpCode);
    error_log("MinIO Upload Response: " . $response);
    ...
    if ($httpCode === 200) {
        ...
        $fileUrl = rtrim($minioHost, '/') . '/' . $minioBucketName . '/' . $uniqueFileName;
        error_log("Successfully uploaded file to: " . $fileUrl);
        return $fileUrl;
    } else {
        ...
        error_log("MinIO Upload Error: " . $message);
