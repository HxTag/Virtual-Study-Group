...
switch ($field) {
    ...
    case 'gender':
        ...
        break;
    case 'geo_location':
        // Validate geo location value (case-insensitive)
        $value = trim($value);
        error_log("Received geo location value: [" . $value . "]"); // Debug log with brackets to see whitespace
        error_log("Value length: " . strlen($value)); // Debug log to check string length
        $isValid = false;
        
        // Debug log all allowed locations
        error_log("Allowed locations: " . implode(", ", $allowed_geo_locations));
        
        foreach ($allowed_geo_locations as $location) {
            error_log("Comparing: [" . $value . "] with [" . $location . "]"); // Debug log with brackets
            error_log("strcasecmp result: " . strcasecmp($value, $location)); // Debug log comparison result
            if (strcasecmp($value, $location) === 0) {
                $isValid = true;
                $value = $location; // Use the exact case from allowed locations
                error_log("Match found! Using value: " . $value);
                break;
            }
        }
        if (!$isValid) {
            error_log("Invalid geo location: [" . $value . "]");
            echo json_encode([
                'success' => false, 
                'message' => 'Invalid geo location selection.',
                'debug' => [
                    'received_value' => $value,
                    'allowed_locations' => $allowed_geo_locations
                ]
            ]);
            exit();
        }
